...
namespace Wurk.Areas.Administration.Controllers
{
    ...
    public class UserController : AdministrationController
    {
        ...
        {
            var user = await userService.GetApplicationUser<ApplicationUser>(id);
        ...
        public async Task<IActionResult> Roles(UserRolesViewModel model)
        {
            ...
            var user = await userService.GetApplicationUser<ApplicationUser>(model.UserId);
            var userRoles = await userManager.GetRolesAsync(user);
            await userManager.RemoveFromRolesAsync(user, userRoles);

            if (model.RoleNames?.Count() > 0)
            {
                userRoles.Add(model.RoleNames.FirstOrDefault());
